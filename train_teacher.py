...
def create_teacher_model(img_height=28, img_width=28):
    ...
    # Output layer
    outputs = Dense(10, activation='softmax')(x)  # Modify the number of units in the output layer
...
def plot_loss_and_accuracy(train_loss, val_loss, train_acc, val_acc, epoch):
    ...
    # Save the plot
    plot_path = f'./teacher_training_plots/epoch_{epoch + 1}.png'
    os.makedirs('./teacher_training_plots', exist_ok=True)  # Ensure the directory exists
...
def plot_confusion_matrix(y_true, y_pred, epoch):
    ...
    # Automatically adjust layout to ensure labels are not cut off
    plt.tight_layout()
    
    plot_path = f'./teacher_confusion_matrices/epoch_{epoch + 1}.png'
    os.makedirs('./teacher_confusion_matrices', exist_ok=True)  # Ensure the directory exists
    plt.savefig(plot_path)
    plt.close()
...
def test_model(model, X_test, y_test, checkpoint_filepath):
    ...
    plt.savefig('./teacher_test_confusion_matrix.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    epochs = 1000
    batch_size = 128
    checkpoint_filepath = './best_teacher_weights.h5'
